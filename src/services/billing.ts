...
/**
 * The charge heads a forwarder actually bills, with the SAC each one usually
 * goes out under.
 *
 * These are defaults to pick from, not law. Every one of them is editable on
 * the line, because which SAC a particular desk files a particular charge under
 * is between that desk and its auditor, and a list baked into a CRM should not
 * be the reason an invoice goes out with the wrong code on it.
 *
 * Picking from a list rather than typing is the whole point: the reference
 * system has a blank text box, which is how the same charge ends up as "Doc
 * charges", "Documentation" and "DOC FEE" on three invoices to one customer.
 */
